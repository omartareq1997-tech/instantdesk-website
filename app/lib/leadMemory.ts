...
function detectObjections(text: string): string[] {
  ...
  if (/\b(too small|too big|too far|wrong area|wrong location|not the right)\b/i.test(text)) found.push('property mismatch')
  return found
}
...
function detectViewedProperties(messages: { role: string; content: string }[]): string | null {
  ...
  const propRe = /\b(?:property|apartment|flat|unit|listing)\s+(?:at|on|in)\s+([A-Z][^\n.!?]{4,50})/gi
  for (const m of messages) {
    if (m.role !== 'user') continue
    let match: RegExpExecArray | null
    while ((match = urlRe.exec(m.content)) !== null) refs.push(match[0].slice(0, 80))
    while ((match = propRe.exec(m.content)) !== null) refs.push(match[1].trim().slice(0, 60))
  }
...
export function formatMemoryForPrompt(memory: LeadMemory | null): string {
  ...
  if (memory.viewed_properties)  lines.push(`Viewed/interested: ${memory.viewed_properties}`)
