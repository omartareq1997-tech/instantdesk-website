...
async function stubChatApi(page: import('@playwright/test').Page) {
  const storedMessages: { id: string; role: string; content: string; metadata?: Record<string, unknown> }[] = []
  let count = 0
  ...
  await page.route('**/api/chat', async route => {
    ...
    count += 1
    storedMessages.push(
      { id: `u-${count}`, role: 'user', content: body.message ?? '' },
      { id: `a-${count}`, role: 'assistant', content: `Reply ${count}: ${body.message ?? ''}` },
    )
    await route.fulfill({
      status: 200,
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ reply: `Reply ${count}: ${body.message ?? ''}`, conversation_id: conversationId }),
    })
  })
